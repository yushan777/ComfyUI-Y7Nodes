...
descriptions = {
    ...
    "Y7Nodes_Flux2KleinEdit_MultiRef": [
        ...
        short_desc("Edit a picture with Flux.2 Klein, with up to seven extra reference pictures for the model to look at."),
        normal("Pick the picture you want to edit on the node itself. Paint a mask on it if you only want to change one area. Add extra pictures if you want the model to copy a face, an outfit, a style or a product from somewhere else."),
        normal("It does the job of a whole row of nodes — Load Image, a resize, VAE Encode, mask tidying and the conditioning nodes — on its own."),
        normal("Getting started:"),
        normal("- Connect a `vae`, connect your `positive` and `negative` prompts, and send all three of the node\'s main outputs on to your sampler", 1),
        normal("- Choose your picture in the `image` box. Use the upload button if it is not in your input folder yet", 1),
        normal("- To change just one area: right-click the node → Open in MaskEditor, paint over the part you want changed, and save. Skip this and the whole picture is free to change", 1),
        normal("- To add a reference: connect any image into `ref_image_2`. A new empty socket appears each time you fill the last one, up to `ref_image_8`", 1),
        normal("Talking about your pictures in the prompt:"),
        normal("- The model sees your pictures as a numbered list. The one on the node is number 1, `ref_image_2` is number 2, `ref_image_3` is number 3, and so on — the socket names match the numbers", 1),
        normal("- Refer to them by number in ordinary English, and always add a noun: \"the man in Figure 1\", \"the jacket from Figure 2\", \"the room in image 3\". A bare number has nothing to hold on to", 1),
        normal("- \"Figure 1\" and \"image 1\" both work — they are just ordinary words to the model. If it latches onto the wrong picture, keep the same seed and try the other wording", 1),
        normal("- There is no special code to type. `<image1>`, `[Image 1]` and `@ref2` do nothing at all. The part of the model that reads your prompt never sees the pictures — they reach it by a different route", 1),
        normal("- Say what to keep as well as what to change: \"...keep her face and hairstyle from Figure 1\". References are suggestions, not rules, and nothing forces the model to follow them", 1),
        normal("Picture sizes — the two megapixel settings:"),
        normal("- A megapixel is a million pixels. Setting `1.0` resizes a picture until it is made of about a million pixels, keeping its shape. A big 4000×3000 photo shrinks to 1184×880; a small 600×600 logo grows to 1024×1024. Nothing is ever stretched or squashed", 1),
        normal("- `target_megapixels` is for the picture you are editing, and it decides how big your finished picture comes out", 1),
        normal("- `ref_megapixels` is for the reference pictures. They are only there to be looked at, so their size does not affect the size of your result", 1),
        normal("- Both work the same way: `0` means leave those pictures at whatever size they already are, and anything higher is the size to aim for", 1),
        normal("- The setting applies to each picture separately, so they add up. One picture plus six references, all at `1.0`, is seven megapixels of work — not one. That is what uses your video memory and your time", 1),
        normal("- So if you run slow or run out of memory, turn `ref_megapixels` down first — try `0.5`. A face or a style still comes through fine at half the size, and your finished picture stays exactly as big as it was. Turning `target_megapixels` down would shrink the result instead", 1),
        normal("- Klein is built for around `1.0`. Going much higher usually makes things worse rather than better, especially with the fast (distilled) models", 1),
        normal("Inputs:"),
        normal("- `vae`: The VAE from your checkpoint, or from a separate VAE loader", 1),
        normal("- `image`: The picture you are editing. This is the only one you can paint a mask on, and it is always number 1 in the list", 1),
        normal("- `target_megapixels`: How big to make the picture you are editing. Sets the size of your finished picture. `0` leaves it alone", 1),
        normal("- `ref_megapixels`: How big to make each reference picture. Does not affect your output size. `0` leaves them alone", 1),
        normal("- `crop_2_nearest_16px`: Trims a few pixels off the edges so the width and height divide by 16, which Flux.2 works best with. Does nothing if they already do", 1),
        normal("- `expand_mask`: Grows the painted area outwards by this many pixels, so the change covers a little more than you painted. `0` turns it off", 1),
        normal("- `feather_mask`: Softens the edge of the painted area so the change blends in more smoothly. `0` turns it off", 1),
        normal("- `binary_mask`: Makes the mask edge hard again after expanding and feathering, for a crisp cut instead of a soft blend", 1),
        normal("- `ref_image_2` … `ref_image_8` (optional): Extra pictures for the model to look at. Connect one and another empty socket appears, up to seven in total. You cannot paint masks on these", 1),
        normal("- `positive` / `negative` (optional): Your prompts. The node adds the picture information to them and passes them out again", 1),
        normal("Outputs:"),
        normal("- `reference_latent`: The picture you are editing, ready for the sampler. Connect it to your sampler\'s latent input", 1),
        normal("- `positive` / `negative`: Your prompts with the picture information added. Connect these to the sampler as well", 1),
        normal("- `preview_image`: The edited picture after resizing and cropping, so you can see exactly what the model was given", 1),
        normal("- `preview_mask`: The mask after expanding, feathering and hardening. All black if you did not paint one", 1),
        normal("- `ref_count`: How many pictures the model is looking at, counting the one you are editing. A quick way to check your sockets really are connected", 1),
        normal("Good to know:"),
        normal("- Masking is optional. With no mask, the whole picture is free to change", 1),
        normal("- You can only mask the picture on the node, never a reference. That is how the model works, not a missing feature: the mask has to line up exactly with the picture being edited, and a reference is a different picture at a different size, so there would be nothing to line it up with", 1),
        normal("- If a socket carries a batch of images, each image in it counts as its own separate reference", 1),
        normal("- Making a small reference bigger does not add detail that was never there — it just costs more time. If a reference is a logo, some text or line art, a lower `ref_megapixels` closer to its real size often looks better", 1),
    ],
