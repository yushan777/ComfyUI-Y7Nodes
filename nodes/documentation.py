...
descriptions = {
    ...
    "Y7Nodes_QwenVL": [
        "Qwen3-VL",
        short_desc("Run vision-language inference using Qwen3-VL models. Supports image analysis and text-only queries."),
        normal("Uses Qwen3-VL integrated with ComfyUI's memory manager for proper VRAM coordination alongside other loaded models."),
        normal("Models:"),
        normal("- `Qwen/Qwen3-VL-2B-Instruct`", 1),
        normal("- `Qwen/Qwen3-VL-4B-Instruct`", 1),
        normal("- `Qwen/Qwen3-VL-8B-Instruct` (default)", 1),
        normal("- `Qwen/Qwen3-VL-32B-Instruct`", 1),
        normal("Inputs:"),
        normal("- `image`: Optional image input — omit for text-only queries", 1),
        normal("- `model_name`: Which Qwen3-VL model to use", 1),
        normal("- `preset_prompt`: Built-in instruction for how the model should analyse the image", 1),
        normal("- `custom_prompt`: If filled, replaces the preset instruction entirely", 1),
        normal("- `max_new_tokens`: Maximum tokens to generate (64–4096, default 512)", 1),
        normal("- `temperature`: Controls randomness (0.0–2.0, default 0.7)", 1),
        normal("- `top_p`: Nucleus sampling threshold (0.0–1.0, default 0.9)", 1),
        normal("- `repetition_penalty`: Penalises repeated tokens (1.0–2.0, default 1.1)", 1),
        normal("- `seed`: Random seed for reproducibility", 1),
        normal("- `keep_model_loaded`: Keep the model in VRAM between runs to skip reloading", 1),
        normal("- `download_model`: Automatically download the selected model from HuggingFace if not found locally", 1),
        normal("Preset Instructions:"),
        normal("- `Tags`: Up to 50 comma-separated visual tags for text-to-image AI", 1),
        normal("- `Simple Description`: A single concise sentence about the main subject and setting", 1),
        normal("- `Detailed Description`: Paragraph covering subject, environment, lighting, and composition", 1),
        normal("- `Ultra Detailed Description`: Extended paragraph with micro-details, textures, and lighting analysis", 1),
        normal("- `Cinematic Description`: Film-still style paragraph with camera language and mood", 1),
        normal("- `Detailed Analysis`: Structured sections — Subject, People, Environment, Lighting, Camera/Composition, Color/Texture", 1),
        normal("- `Short Story`: A short imaginative story inspired by the image", 1),
        normal("- `Prompt Refine & Expand`: Refines and expands a text prompt for creative image generation", 1),
        normal("Output:"),
        normal("- `response`: The model's text response", 1),
        normal("Models are stored in `models/LLM/<model-name>`. Enable `download_model` to download automatically if not found locally."),
    ],
    ...
    "Y7Nodes_JoyCaption_ExtraOptions": [
        ...
        short_desc("Optional caption modifiers — connect to the JoyCaption node to refine what the model includes or excludes in its output."),
        normal("Each toggle appends an instruction to the caption prompt. Enable only the options relevant to your use case."),
        normal("Available options:"),
        normal("- `Exclude People Info`: Omit fixed attributes (ethnicity, gender) but keep changeable ones (hair, clothing)", 1),
        normal("- `Include Lighting`: Describe the lighting conditions", 1),
        normal("- `Include Camera Angle`: Describe the camera angle", 1),
        normal("- `Include Watermark`: Note whether a watermark is present", 1),
        normal("- `Include JPEG Artifacts`: Note whether JPEG artifacts are present", 1),
        normal("- `Include EXIF`: Describe likely camera settings (aperture, shutter speed, ISO, etc.)", 1),
        normal("- `Exclude Sexual`: Keep the caption PG — no sexual content", 1),
        normal("- `Exclude Image Resolution`: Do not mention image resolution", 1),
        normal("- `Include Aesthetic Quality`: Rate the subjective aesthetic quality (low to very high)", 1),
        normal("- `Include Composition Style`: Describe composition (leading lines, rule of thirds, symmetry, etc.)", 1),
        normal("- `Exclude Text`: Do not mention any text visible in the image", 1),
        normal("- `Specify Depth Field`: Describe depth of field and background focus", 1),
        normal("- `Specify Lighting Sources`: Mention likely artificial or natural light sources", 1),
        normal("- `Do Not Use Ambiguous Language`: Avoid vague or ambiguous phrasing", 1),
        normal("- `Include NSFW`: State whether the image is SFW, suggestive, or NSFW", 1),
        normal("- `Only Describe Most Important Elements`: Focus only on the most prominent elements", 1),
        normal("- `Do Not Include Artist Name or Title`: Omit artist name and artwork title", 1),
        normal("- `Identify Image Orientation`: Note portrait, landscape, or square orientation and aspect ratio", 1),
        normal("- `Include Character Age`: Describe the ages of people/characters when applicable", 1),
        normal("- `Include Camera Shot Type`: Specify shot type (close-up, medium shot, wide shot, etc.)", 1),
        normal("- `Exclude Mood Feeling`: Do not describe mood or emotional tone", 1),
        normal("- `Include Camera Vantage Height`: Specify vantage height (eye-level, bird's-eye, worm's-eye, etc.)", 1),
        normal("- `Mention Watermark`: Explicitly mention any watermark present", 1),
        normal("- `Avoid Meta Descriptive Phrases`: Skip phrases like 'This image shows…' for cleaner T2I prompts", 1),
        normal("- `Refer Character Name`: Refer to people/characters by the name entered in `character_name`", 1),
        normal("- `character_name`: Name used when `Refer Character Name` is enabled", 1),
    ],

    "Y7Nodes_JoyCaption": [
        "JoyCaption",
        short_desc("Generate image captions using a JoyCaption LLaVA model, with full control over caption style, length, and generation parameters."),
        normal("Only HuggingFace JoyCaption models are supported."),
        normal("Models (downloaded automatically to `models/LLM/` on first use):"),
        normal("- `joycaption-beta-one-fp8`: FP8 Dynamic quantization variant", 1),
        normal("- `joycaption-beta-one`: Latest beta release (default)", 1),
        normal("- `joycaption-alpha-two`: Previous alpha release", 1),
        normal("Inputs:"),
        normal("- `image`: The image to caption", 1),
        normal("- `model`: Which JoyCaption model to use", 1),
        normal("- `quantization`: Memory precision mode — `Full Precision (bf16)`, `Balanced (8-bit)`, or `Maximum Savings (4-bit)`", 1),
        normal("- `prompt_style`: Caption style — `Descriptive`, `Descriptive (Casual)`, `SDXL`, `Flux.2`, `MidJourney`, `Danbooru tag list`, `Art Critic`, `Product Listing`, `Social Media Post`", 1),
        normal("- `caption_length`: Target caption length — `any`, `very short`, `short`, `medium`, `long`, `very long`", 1),
        normal("- `max_new_tokens`: Maximum tokens to generate (1–2048, default 512)", 1),
        normal("- `temperature`: Controls randomness (0.0–2.0, default 0.6)", 1),
        normal("- `top_p`: Nucleus sampling threshold (0.0–1.0, default 0.9)", 1),
        normal("- `top_k`: Top-k sampling limit (0 = disabled, range 0–100)", 1),
        normal("- `seed`: Random seed for reproducible results", 1),
        normal("- `custom_prompt`: If filled, replaces the built-in prompt entirely", 1),
        normal("- `memory_management`: How to handle model loading between runs:", 1),
        normal("-  `Keep in Memory`: Model stays loaded for the next run (fastest for repeated use)", 2),
        normal("-  `Clear After Run`: Frees VRAM immediately after each run", 2),
        normal("-  `Global Cache`: Shared model cache across multiple JoyCaption node instances", 2),
        normal("- `extra_options`: Optional — connect a JoyCaption Extra Options node to add caption modifiers", 1),
        normal("Outputs:"),
        normal("- `PROMPT`: The prompt sent to the model (useful for debugging or logging)", 1),
        normal("- `STRING`: The generated caption text", 1),
    ],
    ...
    "Y7Nodes_ImageBatchPath": [
        ...
        normal("Designed to pair with CaptionSaver: the `IMAGE_PATH` output tells CaptionSaver where to write each .txt file, and `IMAGE` feeds into JoyCaption (or any other VLM) for captioning."),
        normal("Inputs:"),
    ...
    "Y7Nodes_CaptionSaver": [
        ...
        normal("Designed to pair with ImageBatchPath and JoyCaption (or any VLM that outputs a STRING): connect `IMAGE_PATH` from ImageBatchPath and the caption `STRING` from JoyCaption."),
        normal("Compatible with any node that outputs a STRING — not limited to JoyCaption. Examples: Florence2, MiniCPM, LLaVA, Qwen-VL, etc."),
