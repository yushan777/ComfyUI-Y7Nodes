...
- preserve the subject phrase exactly.
...
class Y7Nodes_PromptEnhancerFlux:
   ...
   # ==================================================================================
    ...
    def enhance(self, **kwargs):
        ...
        try:
            ...
            if not keep_model_loaded:
                ...
                if llm_name in ModelCache.loaded_models:
                    del ModelCache.loaded_models[llm_name]
                    del ModelCache.loaded_tokenizers[llm_name]
                ...
                ModelCache.loaded_models[llm_name] = llm_model
                ModelCache.loaded_tokenizers[llm_name] = llm_tokenizer
        ...
        if not os.path.exists(full_model_path):
            ...
            if "Llama-3.2-3B-Instruct" in model_repo_name:
                ...
                if missing_files:
                    ...
                    print(f"✅ All required files for {model_repo_name} already exist", color.BRIGHT_GREEN)
            else:
    ...
    def down_load_llm_model(self, llm_name, load_device):
        ...
        # ==================================================================================
        if llm_name in ModelCache.loaded_models and llm_name in ModelCache.loaded_tokenizers:
            print(f"Using cached model {llm_name} from previous run", color.BRIGHT_GREEN)
            return ModelCache.loaded_models[llm_name], ModelCache.loaded_tokenizers[llm_name]
            
        model_path = self.model_path_download_if_needed(llm_name)
        
        # Check if model_path is None (indicating a gated model error)
        if model_path is None:
            # Handle the gated model error gracefully
            raise RuntimeError(f"Cannot use {llm_name} as it is a gated model requiring special access permissions from Hugging Face.")
...
# # Function Call Seqeunce
# # Y7FluxPromptEnhancer.enhance()
# #   ├── model_path_download_if_needed() - Checks if model exists, downloads if needed
# #   │
# #   ├── down_load_llm_model() - Loads model from disk
# #   │   └── model_path_download_if_needed() - Called again from within
# #   │
# #   ├── FluxPromptEngine.__init__() - Creates the enhancer class
# #   │   └── get_model_size() - Calculates model memory requirements
# #   │
# #   ├── comfy.model_patcher.ModelPatcher() - Creates model patcher
# #   │
# #   ├── comfy.model_management.free_memory() - Ensures GPU has space
# #   │
# #   ├── comfy.model_management.load_model_gpu() - Loads model to GPU
# #   │
# #   ├── flux_prompt_enhancer.model() (i.e., FluxPromptEngine.forward())
...
# _MAX_NEW_TOKENS=2048
# MODELS_PATH_KEY = "LLM"
# DEFAULT_PROMPT = "a cinematic still of a woman"
...
#                 "temperature": (
#                     "FLOAT",
#                     {"default": 0.7, "min": 0.1, "max": 2.0, "step": 0.1, "tooltip": "Controls randomness: Higher values produce more diverse outputs"}
#                 ),
#                 "top_p": (
#                     "FLOAT",
#                     {"default": 0.9, "min": 0.8, "max": 1.0, "step": 0.01, "tooltip": "Nucleus sampling: limits tokens to the most probable ones"}
#                 ),
#                 "top_k": (
#                     "INT",
#                     {"default": 40, "min": 22, "max": 100, "tooltip": "Limits token selection to the k most likely next tokens"}
#                 ),
#                 "seed": (
#                     "INT", 
#                     {"default": 0, "min": 0, "max": 0xffffffffffffffff}
...
#     # ==================================================================================
#     def enhance(self, prompt, llm_name, seed=0, temperature=0.7, top_p=0.9, top_k=40, keep_model_loaded=False):
#         # Default prompt if empty
#         if not prompt.strip():
#             prompt = DEFAULT_PROMPT
...
#             # Calculate model size for memory management and handle device placement
#             model_size = sum(p.numel() * p.element_size() for p in llm_model.parameters())
#             model_size += sum(b.numel() * b.element_size() for b in llm_model.buffers())
#             model_size += 1073741824  # Add 1GB extra as buffer
...
#             # Generate prompts directly without going through FluxPromptEngine
#             t5_clip_prompts = generate_flux_t5_clip_prompts(
#                 llm_model, 
#                 llm_tokenizer, 
#                 prompt, 
...
# # ==================================================================================
# class FluxPromptEngine(torch.nn.Module):
#     def __init__(
#         self,
#         llm_model,
#         llm_tokenizer,
...
#         # For regular PyTorch models, calculate based on parameters and buffers
#         self.model_size = self.get_model_size(self.llm_model) + 1073741824  # Add 1GB extra as buffer

#     def forward(self, prompt, seed, max_new_tokens=_MAX_NEW_TOKENS):
#         t5_clip_prompts = generate_flux_t5_clip_prompts(
#             self.llm_model,
#             self.llm_tokenizer,
#             prompt,
...
#         # Return the first pair of prompts (t5, clip)
#         return t5_clip_prompts[0]
...
# # required file for mistral_7b_instruct_v0_2
# # mistral_7b_instruct_v0_2_req_file = "mistral-7b-instruct-v0.2.Q8_0.gguf"
# # mistral_7b_instruct_v0_2_req_file = "mistral-7b-instruct-v0.2.Q6_K.gguf"
# # mistral_7b_instruct_v0_2_req_file = "mistral-7b-instruct-v0.2.Q5_K_M.gguf"
# mistral_7b_instruct_v0_2_req_file = "mistral-7b-instruct-v0.2.Q4_K_M.gguf"
...
#                 elif "Mistral-7B-Instruct-v0.2-GGUF" in model_repo_name:
#                     print(f"ℹ️ Downloading {model_repo_name} (Q4_K_M version, ≈4.4GB)", color.BRIGHT_BLUE)
#                     allow_patterns = [mistral_7b_instruct_v0_2_req_file]
...
#             elif "Mistral-7B-Instruct-v0.2-GGUF" in model_repo_name:
#                 # check if mistral-7b-instruct-v0.2.Q4_K_M.gguf exists in the directry. if not, download that file
#                 required_file = mistral_7b_instruct_v0_2_req_file
                
#                 if not os.path.exists(os.path.join(full_model_path, required_file)):
#                     print(f"ℹ️ Found {model_repo_name} directory but missing {required_file}", color.YELLOW)
#                     print(f"⬇️ Downloading {required_file} for {model_repo_name} (≈4.4GB).\nThis may take a few minutes.", color.YELLOW)
#                     try:
#                         snapshot_download(
#                             repo_id=model_repo_name,
#                             local_dir=full_model_path,
...
#                         )
#                         print(f"✅ {required_file} for {model_repo_name} downloaded successfully!", color.BRIGHT_GREEN)
#                     except Exception as e:
#                         print(f"❌ Error downloading {required_file} for {model_repo_name}: {str(e)}", color.BRIGHT_RED)
#                         raise
#                 else:
#                     print(f"✅ Required file {required_file} for {model_repo_name} already exists", color.BRIGHT_GREEN)
#             else:
#                 # For other models, just inform user that the directory exists
#                 print(f"ℹ️ Model directory for {model_repo_name} already exists", color.BRIGHT_BLUE)
...
#             # Check if this is the Mistral-7B-Instruct-v0.2-GGUF model
            
#             if "Mistral-7B-Instruct-v0.2-GGUF" in llm_name:                
#                 gguf_file = os.path.join(model_path, mistral_7b_instruct_v0_2_req_file)
#                 print(f"Loading GGUF model from {gguf_file}", color.BLUE)
...
#                 # Create a simple wrapper for the tokenizer to match the expected interface
...
#                         # Create a dummy object that mimics the expected interface
#                         class DummyEncodingOutput:
#                             def __init__(self, texts):
#                                 self.texts = texts
#                                 self.input_ids = [[0] * 10] * len(texts)  # Dummy input IDs
                                
#                             def to(self, device):
#                                 return self
                        
#                         dummy_output = DummyEncodingOutput(texts)
#                         return dummy_output
                    
#                     def batch_decode(self, token_ids, skip_special_tokens=True):
#                         # We don't actually use the token IDs since the Llama model handles generation differently
#                         # This is just a placeholder to match the expected interface
#                         return [""] * len(token_ids)
                
#                 llm_tokenizer = LlamaTokenizerWrapper(llm_model)
#             else:
#                 # Load regular transformers model
#                 llm_model = AutoModelForCausalLM.from_pretrained(
#                     model_path,
#                     torch_dtype=torch.bfloat16,
#                 )
...
#             # Calculate model size for memory management and handle device placement
#             if isinstance(llm_model, Llama):
#                 # For GGUF models, we don't need to manage memory or devices in the same way
#                 # The llama_cpp library handles this internally
#                 # Just use a reasonable estimate for memory usage
#                 model_size = 4 * 1024 * 1024 * 1024  # 4GB estimate
                
#                 # Free memory before using the model
#                 comfy.model_management.free_memory(model_size, comfy.model_management.get_torch_device(),)
                
#                 # No need to move the model to a device, llama_cpp handles this
#             else:
#                 # For regular transformers models, calculate size and manage devices
#                 model_size = sum(p.numel() * p.element_size() for p in llm_model.parameters())
#                 model_size += sum(b.numel() * b.element_size() for b in llm_model.buffers())
#                 model_size += 1073741824  # Add 1GB extra as buffer
...
#             # Generate prompts directly without going through FluxPromptEngine
#             t5_clip_prompts = generate_flux_t5_clip_prompts( llm_model, llm_tokenizer, prompt, seed, temperature=temperature, top_p=top_p, top_k=top_k)
...
#                     # Only PyTorch models need to be moved to a different device
#                     llm_model.to(offload_device)
...
#         # Handle device attribute based on model type
#         if isinstance(llm_model, Llama):
#             # GGUF models don't have a device attribute
#             self.device = "cuda" if torch.cuda.is_available() else "cpu"
#         else:
#             # Regular PyTorch models have a device attribute
#             self.device = llm_model.device
            
#         # Calculate model size
#         if isinstance(llm_model, Llama):
#             # For GGUF models, use a reasonable estimate
#             self.model_size = 4 * 1024 * 1024 * 1024  # 4GB estimate
#         else:
#             # For regular PyTorch models, calculate based on parameters and buffers
#             self.model_size = self.get_model_size(self.llm_model) + 1073741824  # Add 1GB extra as buffer
...
#         prompt: Input prompt(s) to enhance
#         seed: Random seed for reproducibility
#         max_new_tokens: Maximum number of new tokens to generate (defaults to _MAX_NEW_TOKENS)
#         temperature: Controls randomness of outputs
#         top_p: Nucleus sampling parameter
#         top_k: Limits token selection to k most likely tokens
...
#     # Only move to device for PyTorch models, not for GGUF models
#     if not isinstance(prompt_enhancer_model, Llama) and hasattr(prompt_enhancer_model, 'device'):
#         model_inputs = model_inputs.to(prompt_enhancer_model.device)
...
#     Returns: List of tuples containing (t5_prompt, clip_prompt) pairs
#     """
#     # Check if we're using a GGUF model (llama_cpp.Llama)
#     if isinstance(prompt_enhancer_model, Llama):
#         # For GGUF models, we use the llama_cpp API
#         decoded_responses = []
#         for text in model_inputs.texts:
#             # Generate text using the llama_cpp API
#             response = prompt_enhancer_model(
#                 text,
#                 max_tokens=_MAX_NEW_TOKENS,  # Using the passed parameter (must be a named parameter)
#                 temperature=temperature,      # Randomness: must be > 0
#                 top_p=top_p,           # Nucleus sampling adds variability
#                 top_k=top_k,             # Optional: restrict to top-K tokens                
#                 stop=["</s>", "<s>"],  # Stop tokens
#                 seed=seed
#             )
...
#     else:
#         # For regular transformers models, use the standard API
#         with torch.inference_mode():
#             outputs = prompt_enhancer_model.generate(
#                 **model_inputs, 
#                 max_new_tokens=_MAX_NEW_TOKENS,  #  (must be a named parameter)
#                 do_sample=True,
#                 temperature=temperature,
#                 top_p=top_p,
#                 top_k=top_k                
...
#         if "T5 Prompt" in response and "CLIP Prompt" in response:
#             # print("T5 Prompt and CLIP Prompt markers present in response", color.BRIGHT_GREEN)
